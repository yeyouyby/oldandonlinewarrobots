...
- **存档由服务器保管**（`data/profiles.json`，可用 `WR_DATA_DIR` 环境变量改目录）：浏览器只保存一个随机 token（localStorage）。所有购买 / 升级 / 装配 / 出战阵容都由服务器校验，客户端无法伪造机甲、等级或货币。清除浏览器数据会丢失 token，等于换了个新账号。同一账号同时只能有一个战斗会话：新开的标签页会接管当前对局（机甲、已用槽位、战绩原样保留，旧标签页被断开），战斗中不能改机库。

可选环境变量：`WR_MAX_PROFILES`（存档上限，默认 20000，满了会淘汰从未做过任何操作的空白账号）、`WR_CREATE_PER_IP`（每 IP 每 10 分钟可新建账号数，默认 5）、`TRUST_PROXY=1`（部署在反向代理后时取 `X-Forwarded-For` 作为客户端 IP）。
